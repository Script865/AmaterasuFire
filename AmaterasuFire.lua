...
-- إعدادات القدرة
local DAMAGE = 1000
local FIRE_DURATION = 10 -- النار تختفي بعد 10 ثواني
...
button.Size = UDim2.new(0,100,0,50) -- نفس الحجم
button.Position = UDim2.new(0.75,0,0.85,0) -- نفس مكان الصف السفلي يمين
...
local function castAmaterasu(owner)
    ...
    local rayDirection = camera.CFrame.LookVector * 1000 -- مسافة طويلة (شبه لانهائية)
    ...
    if result then
        ...
        emitter.Texture = "rbxassetid://243660364" -- شكل النار
        ...
        game:GetService("Debris"):AddItem(firePart, FIRE_DURATION)
    end
end
